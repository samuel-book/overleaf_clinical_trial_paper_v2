...
We trained a set of six independent XGBoost models \cite{chen_xgboost_2016} to predict the likelihood of an ischaemic stroke patient attaining any given mRS threshold at discharge (where achieving a lower threshold is better). The model of mRS $\le$ 5 also provides a model of survival vs. death. The model of mRS $\le$ also provides a model of death vs. survival. The results from the test set for the model fitted on the first k-fold split was used to report the relationships between feature values and their contribution to the predictions.
